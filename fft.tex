...
\begin{algorithm}[H]
  ...
  \KwData{ $a = [a_0, a_1, ... a_{N-1}] \in \mathbb{C}^N$ - wektor liczb zespolonych długości N,
           $X = [x_0, ..., x_{N-1}] \in \mathbb{C}^N$ spełniający równość $|{x^2 | x \in X}| = \frac{|X|}{2}$ dla każdego wywołania rekurencyjnego.}

  \KwResult{$y \in \mathbb{C}^n$, taki, że $y_k = \sum_{n=0}^{N-1}a_n \cdot \exp{\frac{-2\pi i n k}{N}}$ dla $k = 0,1,...,N-1$}
  function fft(a, X):
